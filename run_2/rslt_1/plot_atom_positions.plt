...
plot "./atom_positions.DAT" using ($1):(($7-$8)*lca)\
    with lp lt 1 ps 1.5 pt 9 lc rgb "blue" title "Ti>O (relaxed, O below Ti)",\
    "./atom_positions.DAT" using ($1):(($9-$7)*lca)\
    with lp lt 1 ps 1.5 pt 11 lc rgb "dark-blue" title "Ti<O (relaxed, O below Ti)",\
    "./atom_positions.DAT" using ($1):(($3-$4)*lcb)\
     with lp lt 1 ps 1.5 pt 7 lc rgb "dark-green" title "Ti>O (before relaxation)"
